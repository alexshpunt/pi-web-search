...
function httpErrorMessage(status: number, payload: unknown, bodyText: string): string {
  ...
  return detail
    ? `Search failed with HTTP ${status}: ${detail}`
...
function selectOrder(
  ...
  return [
    selected,
    ...providers.map((_provider, index) => index).filter((index) => index !== selected),
  ];
}
...
async function performProviderSearch(
  ...
  const results = normalizeSearchResponse(config.provider, payload).filter((item) =>
    isWebUrl(item.url),
  );
...
export async function performSearch(
  ...
  return {
    ...
    strategy: config.strategy,
    attempts,
    error: `All configured search providers failed: ${attempts
